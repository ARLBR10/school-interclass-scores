...
function getDefaultColumnIcon(key: string): LucideIcon {
  ...
  if (
    ...
    normalizedKey.includes('committee')
  ) {
    return Globe
  }
...
export function DynamicTable<T extends AdminTableRow>({
  ...
  const referencedRowValue = searchParamKey
  ...
  useEffect(() => {
    ...
    const nextRowIndex = tableData.findIndex(
      (row) =>
        resolveSearchParamValue(row, searchParamKey) === referencedRowValue,
    )
        ...
        .map((column) => {
          ...
          return {
            ...
            renderInput: formRender
              ? ({ value, values, onChange, onFieldChange, mode }) =>
                  formRender({
                    value,
                    values,
                    onChange,
  ...
  const cellSortingFn = useMemo<SortingFn<T>>(
    () => (leftRow, rightRow, columnId) =>
      compareCellValues(
        leftRow.getValue(columnId),
        rightRow.getValue(columnId),
      ),
    [],
  )
                    ...
                    <DropdownMenuContent align="end" className="w-40">
                      <DropdownMenuGroup>
                        <DropdownMenuItem
                          disabled={!onUpdate}
                          onSelect={() =>
                            handleOpenEdit(row.original, row.index)
                          }
                        >
                          <Pencil />
                          Editar
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _columnId, filterValue) => {
      ...
      const rowReferenceValues = [
        ...
        searchParamKey
          ? resolveSearchParamValue(row.original, searchParamKey)
          : null,
      ]
      ...
      return tableColumns.some((column) =>
        normalizeSearchValue(
          getColumnSearchValue(row.original, column),
  ...
  function handleOpenEdit(row: T, index: number) {
    const nextReferencedRowValue = searchParamKey
      ? resolveSearchParamValue(row, searchParamKey)
      : null
    ...
    if (
      nextReferencedRowValue &&
      nextReferencedRowValue !== referencedRowValue
    ) {
  ...
  async function handleDeleteRow(row: T, index: number) {
    let shouldApplyChange = true
    const deletedRowReference = searchParamKey
      ? resolveSearchParamValue(row, searchParamKey)
      : null
